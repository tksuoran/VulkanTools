...
const LayerFile *Configurator::FindLayerNamed(QString qsLayerName, const char *location) {
    for (int i = 0; i < available_Layers.size(); ++i) {
        ...
        // Match both name and location or just name
        const bool check_location = layer_file->layer_path == QString(location) || location == nullptr;

        if (!(qsLayerName == layer_file->name && check_location)) continue;
        return layer_file;
    }
